...
# Fungsi untuk menentukan apakah sebuah string adalah vokal atau bukan 
...
# Class untuk menghitung jumlah fitur
@dataclass
class FD:
    name: str
    plus: FDType
...
def parse_token(word : str):
    ...
    for token in tokens:
        # Mencari index vokal pada token
        m = re_vocab.search(token)
        if m is None:
            # Jika tidak terdapat vokal, lewati
            # print(f'word = {word}')
            # print(f"tokens = {tokens}")
            # input()
            continue
        
        # Menentukan posisi awal dan akhir nukleus
        nucleus_start, nucleus_end = m.span()
        # Mengambil onset dari token
        onset = token[:nucleus_start]
        # Mengambil nukleus dari token
        nucleus = token[nucleus_start:nucleus_end]
        # Mengambil coda dari token
...
# Fungsi untuk merubah kata dasar ke dalam bentuk list token
...
# Menghitung kemunculan fitur dari file berisi daftar kata
